...
var getProjectConfig = async (_dir) => {
  const getProjectConfigPath = (__dir) => {
    ...
    const configPath2 = findup(PROJECT_CONFIG_FILE, {
      cwd: projectDir,
      nocase: true
    });
    return configPath2;
...
async function determineSrcAndDest() {
  let absoluteSrc;
  let dest;
  const { projectDir, projectConfig } = await getProjectConfig(void 0);
  if (projectDir && projectConfig) {
    let themeJsonPath = getThemeJSONPath(void 0);
    if (!themeJsonPath) {
...
function getDevServerCommandLineOptions(argv) {
  ...
  const showUI = args.includes("--showUI");
  return {
...
// ../cms-js-internal/dist/types-BXr0V0Zd.js
...
function findCmsModules(buildContentDescriptors = {}) {
  ...
  return {
    ...
    transform(code, id) {
      ...
      if (shouldSkipModule(config, moduleInfo, code))
        return null;
...
function toRelativePath(config, path14) {
  return path14.replace(config.root, "");
}
function shouldSkipModule(config, moduleInfo, code) {
  const isExternal = config.command !== "serve" && moduleInfo.isExternal;
  return isExternal || !moduleInfo.id.startsWith(config.root) || !(code.includes(CMS_MODULE_COMPONENT_NAME) || code.includes(DND_AREA_COMPONENT_NAME));
}
...
function analyzeSourceCodeUsages(ctx, moduleInfo, config, code) {
  ...
  traverse(sourceAst, {
    ImportDeclaration(path14) {
      if (t.isStringLiteral(path14.node.source, {
    ...
    JSXOpeningElement(path14) {
      if (t.isJSXIdentifier(path14.node.name, {
        name: moduleComponentLocalImportName
      })) {
        let cmsModuleName;
        let cmsModulePath;
        let cmsModuleLabel;
        for (const prop of path14.node.attributes) {
  ...
  while (offsetSoFar < body.length && offsetSoFar !== -1) {
    ...
    if (startOffset >= 0) {
      ...
      if (pathEndOffset >= 0 && pathEndOffset < nextNewlineOffset) {
        const path14 = body.substring(
          startOffset + localJsRenderStartString.length,
          pathEndOffset
        );
        offsetSoFar = pathEndOffset + 1;
        if (startOffset >= 0 && path14) {
...
var renderLocalError_default = (path14, error) => {
  const stackWithColorClasses = Converter.toHtml(error.stack);
  return `<div data-js-rendered="error" style="${darkErrorStyles}">Error rendering ${path14}<br/><br/><code><pre>${stackWithColorClasses}</pre></code></div>`;
};
...
async function initialPreviewBufferUpload(accountId, src, dest, filePaths, uploadOptions) {
  ...
  const results = await uploadFolder(accountId, src, dest, {}, rest, filePaths);
  onFinishCallback(results);
}
...
function getDesignManagerPath(src, dest, file) {
  const regex = new RegExp(`^${escapeRegExp2(src)}`);
...
var getPreviewUrl = (request2, queryParams) => {
  const { authedPortalId, authedEnv, authedHublet } = request2.app.locals;
  return `http://${authedPortalId}.hubspotpreview${authedEnv === "qa" ? "qa" : ""}-${authedHublet}.com/_hcms/preview/template/multi?${stringify(queryParams)}`;
};
...
function buildDevFileHashTable(src, dest, filePaths) {
  return filePaths.reduce((acc, filePath) => {
    ...
    const destPath = join3(dest, relativePath);
...
var renderComponentForPlaceholder = (indexForThisRender, localRenderPlaceholder, webRoot2, viteServer, pageUrl, basePath, environmentConfig, hsProjectJsonProjectName, webRootProjectComponent) => {
  ...
  const resolvedPath = normalizePath2(
...
async function renderLocalProjectModulePreview(moduleName, authedPortalId, authedEnv, authedHublet, webRoot2, viteServer, jsRenderingGates, projectName, projectComponentPath, fullOriginalUrl, cookieValue) {
  const modulePath = normalizePath3(
...
var partialRouteHandler = async (request2, response) => {
  ...
  const partialPath = normalizePath4(
    path8.resolve(webRoot2, "components/partials/", partialName)
  );
  const entrypointPath = normalizePath4(
    [
      "@projects",
      projectName,
      projectComponentPath,
...
var moduleRouteHandler = async (request2, response) => {
  ...
  const modulePath = normalizePath5(
...
async function fetchModuleByPath(accountId, path14) {
  return (await hsCliHttp8.get(accountId, {
    url: `${DESIGN_MANAGER_API_PATH}/modules/by-path/${path14}?portalId=${accountId}`
  })).data;
}
async function fetchTemplateByPath(accountId, path14) {
  return (await hsCliHttp8.get(accountId, {
    url: `${DESIGN_MANAGER_API_PATH}/templates/by-path/${path14}?portalId=${accountId}`
  })).data;
}
...
// src/routes/homeRouteHandler.tsx
import { Fragment, jsx as jsx8, jsxs as jsxs4 } from "react/jsx-runtime";
var homeRouteHandler = async (request2, response, next) => {
  const domainToProxy = getSubDomainFromValidLocalDomain(request2.hostname);
  if (domainToProxy) {
    next();
    return;
...
var homeRouteHandler_default = homeRouteHandler;
...
import {
  buildStaticEnvConfig as buildStaticEnvConfig5
} from "@hubspot/cms-components";
...
var templateRouteHandler = async (request2, response) => {
  ...
  try {
    ...
    console.error(`Failed to fetch template data for ${calculatedPath}: ${err}`);
  }
...
var createDevServerRoutes = async (viteServer, storybookPort) => {
  ...
  devServerApp.get("/", homeRouteHandler_default);
...
import { getAccountId as getAccountId3, getEnv as getEnv3 } from "@hubspot/local-dev-lib/config";
import cors from "cors";
import { buildStaticEnvConfig as buildStaticEnvConfig6 } from "@hubspot/cms-components";
function getShadowDevServerCheckHandler(devServerPort, projectName, gates) {
  return async function shadowDevServerCheckHandler(request2, response) {
    ...
    if (query) {
      ...
      try {
        ...
        const { previewKey } = previewInfo;
        const environmentConfig = buildStaticEnvConfig6(
          portalId,
          "na1",
          // bad lazy TF (but does it really matter for this check)
          proxyEnv,
          false,
          gates
        );
        ...
        response.status(200).set({ "Content-Type": "application/json" }).json({ hasJSBuildingBlocks, localProxyUrl });
      } catch (error) {
        response.status(500).end(
          `Failed proxy render of page id = ${contentId} hub id = ${portalId}
...
async function createShadowDevServerRoutes(devServerPort, projectName, gates) {
  const shadowDevServerRoutes = Router2();
...
async function startShadowDevServer(devServerPort, shadowPort, projectName, jsRenderingGates) {
  ...
  return {
    ...
    configureServer: {
      ...
      async handler(server) {
        const cmsComponents3 = await server.ssrLoadModule(
          "@hubspot/cms-components"
        );
        if (cmsComponents3.setSecretsFromEnv) {
          console.log(
            `Attempting to load HS_* secrets from .env: ${secretNames}`
          );
          cmsComponents3.setSecretsFromEnv(secretNames);
...
function getReactModules(themeWebRoot) {
  const { moduleNames } = findAllModulesInRoot(themeWebRoot);
  return moduleNames;
}
function getDevServerUIData(reqLocals) {
  const { themeWebRoot, authedPortalId, projectName } = reqLocals;
  const moduleNames = getReactModules(themeWebRoot);
  return {
    authedPortalId,
    projectName,
    moduleNames
...
var getDevServerUIData_default = getDevServerUIData;

// src/createAppServer.ts
async function createAppServer(webRoot2, reqLocals, port) {
  const app = express2();
  const origin = `http://hslocal.net:${port}`;
  const viteConfig = {
    server: {
      middlewareMode: true,
      host: "hslocal.net",
  ...
  const viteServer = await createViteServer2(viteConfig);
  app.use(viteServer.middlewares);
  app.use("*", async (req, res) => {
    try {
      const inititialProps = getDevServerUIData_default(reqLocals);
      const url3 = req.originalUrl.replace("/", "");
      let template = await fs4.readFile("./app/index.html", "utf-8");
      template = await viteServer.transformIndexHtml(url3, template);
      const { render } = await viteServer.ssrLoadModule(
        "./app/serverEntry.tsx"
      );
      const rendered = await render(inititialProps);
      ...
      viteServer == null ? void 0 : viteServer.ssrFixStacktrace(e);
      console.log(e.stack);
...
async function fetchDomains(portalId) {
  try {
    const result = (await hsCliHttp11.get(portalId, {
      url: `/cms/v3/domains`,
      params: { portalId },
      responseType: "json"
    })).data;
...
function findHSProjectJson(startPath) {
  ...
  let dirLevel = 0;
  const maxDirLevel = 4;
  while (path11.dirname(currentDir) !== currentDir && dirLevel < maxDirLevel) {
    const filePath = path11.join(currentDir, HSPROJECT_JSON);
...
async function validateHSPlatform(projectConfigPath, portalId) {
  ...
  try {
    platformVersionResult = (await hsCliHttp12.get(portalId, {
...
var isInModulesFolder = (filePath, projectRoot) => {
  ...
  const currentModuleId = path12.normalize(filePath);
  const moduleEntryPath = getEntryPathFromId(currentModuleId);
  const subFolderTokens = moduleEntryPath.split(path12.sep);
  return {
...
var getIndexFile = (filePath, projectRoot) => {
  ...
  return indexFileName ? { indexFilePath: path12.join(dir, indexFileName), isRootModuleFile } : void 0;
};
...
var maybeUpdateFields = async (filePath, viteServer, projectRoot) => {
  ...
  const indexFileName = path12.parse(indexFilePath).name;
  ...
  if (fields) {
    ...
    const typeFilePath = path12.join(path12.dirname(filePath), typeFileName);
    ...
    if (!hasAutoGenComment)
      return;
    const moduleFolderName = path12.basename(path12.dirname(filePath));
...
var startFieldsTypeGenWatcher = (viteServer, projectRoot) => {
  ...
  moduleSubpaths.forEach((modulePath) => {
    const resolvedPath = path12.resolve(projectRoot, modulePath);
...
async function createDevServer(webRoot2, storybook2, hubspotCliConfigPathOption2, hubspotCliAccountNameOption2, sslEnabledOption2, fieldGenEnabledOption2, themePreviewOptions2, showUI) {
  ...
  if (validateConfig()) {
    ...
    const env2 = getEnv4(hubspotCliAccountNameOption2);
    if (!env2) {
      throw new Error(
...
Local proxy functionality and module preview routes will not work without setting up authentication. You can work purely on modules locally at "/module/[module_name]", however the experience may be degraded due to missing account data.`
  ...
  let authedHublet;
  try {
    const accountInformation = (await hsCliHttp13.get(authedPortalId, {
  ...
  const absoluteWebRoot = normalizePath7(path13.resolve(process.cwd(), webRoot2));
  ...
  expressServer.use(
    BASE_PATH,
    await createDevServerRoutes_default(viteServer, storybook2 ? STORYBOOK_PORT : null)
  );
...
`
  ...
  if (showUI) {
    const appWebRoot = path13.join(
      path13.dirname(url2.fileURLToPath(import.meta.url)),
      "../app"
    );
    const reqLocals = {
    ...
    const appServer = await createAppServer_default(appWebRoot, reqLocals, PORT + 1);
    httpServers.push(appServer);
  }
  if (storybook2) {
    const packageRoot = path13.dirname(url2.fileURLToPath(import.meta.url));
    const configPath = path13.resolve(packageRoot, "./storybook-main");
    ...
    const storybookProcess = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          hsEnv: authedEnv,
          hublet: authedHublet,
          WEB_ROOT: path13.join(webRoot2, "components")
        }
      }
    );
    console.log(
