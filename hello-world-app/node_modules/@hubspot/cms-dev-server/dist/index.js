...
function toRelativePath(config, path13) {
  return path13.replace(config.root, "");
}
...
function analyzeSourceCodeUsages(ctx, moduleInfo, config, code) {
  ...
  traverse(sourceAst, {
    ImportDeclaration(path13) {
      if (t.isStringLiteral(path13.node.source, {
        value: "@hubspot/cms-components"
      })) {
        for (const specifier of path13.node.specifiers) {
    ...
    JSXOpeningElement(path13) {
      if (t.isJSXIdentifier(path13.node.name, {
        name: moduleComponentLocalImportName
      })) {
        let cmsModuleName;
        let cmsModulePath;
        let cmsModuleLabel;
        for (const prop of path13.node.attributes) {
  ...
  while (offsetSoFar < body.length && offsetSoFar !== -1) {
    ...
    if (startOffset >= 0) {
      ...
      if (pathEndOffset >= 0 && pathEndOffset < nextNewlineOffset) {
        const path13 = body.substring(
          startOffset + localJsRenderStartString.length,
          pathEndOffset
        );
        offsetSoFar = pathEndOffset + 1;
        if (startOffset >= 0 && path13) {
...
var renderLocalError_default = (path13, error) => {
  const stackWithColorClasses = Converter.toHtml(error.stack);
  return `<div data-js-rendered="error" style="${darkErrorStyles}">Error rendering ${path13}<br/><br/><code><pre>${stackWithColorClasses}</pre></code></div>`;
};
...
async function renderLocalProjectModulePreview(moduleName, authedPortalId, authedEnv, authedHublet, webRoot, viteServer, jsRenderingGates, projectName, projectComponentPath, fullOriginalUrl, cookieValue) {
...
async function fetchModuleByPath(accountId, path13) {
  return (await hsCliHttp8.get(accountId, {
    url: `${DESIGN_MANAGER_API_PATH}/modules/by-path/${path13}?portalId=${accountId}`
  })).data;
}
async function fetchTemplateByPath(accountId, path13) {
  return (await hsCliHttp8.get(accountId, {
    url: `${DESIGN_MANAGER_API_PATH}/templates/by-path/${path13}?portalId=${accountId}`
  })).data;
}
...
// src/createAppServer.ts
async function createAppServer(webRoot, reqLocals, port) {
  const app = express2();
  ...
  app.use("*", async (req, res) => {
    try {
      ...
      let template = await fs3.readFile("./app/index.html", "utf-8");
...
import {
  ...
  getAccountConfig as getAccountConfig2,
  loadConfig,
  validateConfig
...
function checkFileExists(filePath) {
  return fs4.existsSync(filePath) && fs4.statSync(filePath).isFile();
}
function findHSProjectJson(startPath) {
  let currentDir = startPath;
  let dirLevel = 0;
  const maxDirLevel = 4;
  while (path10.dirname(currentDir) !== currentDir && dirLevel < maxDirLevel) {
    const filePath = path10.join(currentDir, HSPROJECT_JSON);
...
async function validateHSPlatform(projectConfigPath, portalId) {
  ...
  const hsProjectJSONContents = fs4.readFileSync(projectConfigPath, "utf8");
...
var isInModulesFolder = (filePath, projectRoot) => {
  ...
  const currentModuleId = path11.normalize(filePath);
  const moduleEntryPath = getEntryPathFromId(currentModuleId);
  const subFolderTokens = moduleEntryPath.split(path11.sep);
  return {
...
var fileContainsAutoGenComment = async (filePath) => {
  if (!fs5.existsSync(filePath))
    return true;
  const stream2 = fs5.createReadStream(filePath);
...
var getIndexFile = (filePath, projectRoot) => {
  ...
  return indexFileName ? { indexFilePath: path11.join(dir, indexFileName), isRootModuleFile } : void 0;
};
...
var maybeUpdateFields = async (filePath, viteServer, projectRoot) => {
  ...
  const indexFileName = path11.parse(indexFilePath).name;
  ...
  if (fields) {
    ...
    const typeFilePath = path11.join(path11.dirname(filePath), typeFileName);
    ...
    if (!hasAutoGenComment)
      return;
    const moduleFolderName = path11.basename(path11.dirname(filePath));
...
var startFieldsTypeGenWatcher = (viteServer, projectRoot) => {
  ...
  moduleSubpaths.forEach((modulePath) => {
    const resolvedPath = path11.resolve(projectRoot, modulePath);
...
async function createDevServer(webRoot, storybook, hubspotCliConfigPathOption, hubspotCliAccountNameOption, sslEnabledOption, fieldGenEnabledOption, themePreviewOptions, showUI) {
  ...
  if (validateConfig()) {
    ...
    const env = getEnv4(hubspotCliAccountNameOption);
    if (!env) {
      throw new Error(
    ...
    authedEnv = env;
    personalAccessKey = getAccountConfig2(authedPortalId) && getAccountConfig2(authedPortalId).personalAccessKey;
...
Local proxy functionality and module preview routes will not work without setting up authentication. You can work purely on modules locally at "/module/[module_name]", however the experience may be degraded due to missing account data.`
  ...
  const absoluteWebRoot = normalizePath7(path12.resolve(process.cwd(), webRoot));
  ...
  expressServer.use(
    BASE_PATH,
    await createDevServerRoutes_default(viteServer, storybook ? STORYBOOK_PORT : null)
  );
  if (sslEnabledOption) {
...
`
  ...
  if (showUI) {
    const appWebRoot = path12.join(
      path12.dirname(url2.fileURLToPath(import.meta.url)),
      "../app"
    );
    const reqLocals = {
  ...
  if (storybook) {
    const packageRoot = path12.dirname(url2.fileURLToPath(import.meta.url));
    const configPath = path12.resolve(packageRoot, "./storybook-main");
    ...
    const storybookProcess = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          hsEnv: authedEnv,
          hublet: authedHublet,
          WEB_ROOT: path12.join(webRoot, "components")
        }
      }
    );
    console.log(
